...
export const getMetricDefinitions = async () => {
  return getApplicationMetricDefinitions();
};

export const getMetrics = async (month) => {
  return getApplicationMetrics(month);
};
...
export const saveMetricRecord = async (payload) => {
  const normalizedPayload = {
    ...
    actual_new_graduate:
      payload.actual_value ?? payload.actual_new_graduate ?? 0,
...
// 採用進捗のダミー保存
export const saveRecruitmentPipeline = async () => {
  return { message: "保存しました" };
};

// Web閲覧のダミー保存
export const saveWebViewRecord = async () => {
  return { message: "保存しました" };
};
